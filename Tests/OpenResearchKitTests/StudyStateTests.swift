...
final class StudyStateTests: XCTestCase {
    ...
    func test_shouldDisplayIntroductorySurvey_logic() {
    ...
    func test_surveyUrl_generation() {
        ...
        XCTAssertTrue(components?.queryItems?.contains(URLQueryItem(name: "uuid", value: userID)) ?? false)
        XCTAssertTrue(components?.queryItems?.contains(URLQueryItem(name: "test", value: "true")) ?? false, "Additional query items should be present.")
